...
class TestMapperOnChange(unittest.TestCase):
    ...
    def test_on_change_true_and_value_changes(self):
        register_def = {
            "number": 100, "startbit": 0, "nobits": 16, "signed": False,
            "on_change": True,
            "measurementmapping": {"templatestring": '{"temp": %%}'}
        }

        # First poll: Should always publish
        messages1 = self.mapper.map_register(read_register=[123], register_def=register_def)
        ...
        messages2 = self.mapper.map_register(read_register=[456], register_def=register_def)
    ...
    def test_on_change_true_and_value_is_same(self):
        ...
        messages2 = self.mapper.map_register(read_register=[123], register_def=register_def)
        self.assertEqual(len(messages2), 0, "Should not publish when value is unchanged")
    ...
    def test_on_change_false_and_value_is_same(self):
        ...
        messages2 = self.mapper.map_register(read_register=[123], register_def=register_def)
        self.assertEqual(len(messages2), 1, "Should always publish when on_change is false")
    ...
    def test_on_change_not_present_and_value_is_same(self):
        ...
        messages2 = self.mapper.map_register(read_register=[123], register_def=register_def)
        self.assertEqual(len(messages2), 1, "Should default to on_change=false and always publish")
    ...
    def test_on_change_with_float_values(self):
        ...
        register_def = {
            "number": 102, "startbit": 0, "nobits": 32, "signed": True,
            "on_change": True, "datatype": "float",
            "measurementmapping": {"templatestring": '{"voltage": %%}'}
        }

        # First poll with 123.45
        messages1 = self.mapper.map_register(read_register=list(float_to_regs(123.45)), register_def=register_def)
        self.assertEqual(len(messages1), 1, "Should publish on first poll for float value")
        data1 = json.loads(messages1[0].data)
        self.assertAlmostEqual(data1["voltage"], 123.45, places=5)

        # Second poll, same value
        messages2 = self.mapper.map_register(read_register=list(float_to_regs(123.45)), register_def=register_def)
        self.assertEqual(len(messages2), 0, "Should not publish when float value is the same")

        # Third poll, very close value (should be considered the same by math.isclose)
        close_value = 123.45 + 1e-8
        messages3 = self.mapper.map_register(read_register=list(float_to_regs(close_value)), register_def=register_def)
        self.assertEqual(len(messages3), 0, "Should not publish for very close float values")

        # Fourth poll, different value
        messages4 = self.mapper.map_register(read_register=list(float_to_regs(125.0)), register_def=register_def)
        self.assertEqual(len(messages4), 1, "Should publish when float value changes significantly")
