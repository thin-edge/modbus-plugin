...
class ModbusMapper:
    ...
    def validate(self, registerdef):
        ...
        if fieldlength > 64:
            raise Exception(
                f"definition of field length too long ({fieldlength}) "
                f'for register {registerdef["number"]} at {startbit}'
            )
    ...
    def parse_float(self, buffer, fieldlengt):
        ...
        return struct.unpack(
            formats[fieldlengt], buffer.to_bytes(int(fieldlengt / 8), sys.byteorder)
        )[0]
    ...
    def mapregister(self, readregister, registerdef):
        ...
        registertype = "ir" if registerdef.get("input") else "hr"
        ...
        buffer = self.buffer_register(
            readregister, is_little_endian, is_litte_word_endian
        )
        buflength = len(readregister) * 16
        # shift and mask for the cases where the startbit > 0 and
        ...
        if registerdef.get("measurementmapping") is not None:
            value = (
                value
                * (registerdef.get("multiplier") or 1)
                * (10 ** (registerdef.get("decimalshiftright") or 0))
                / (registerdef.get("divisor") or 1)
            )
            data = registerdef["measurementmapping"]["templatestring"].replace(
                "%%", str(value)
            )
            messages.append(
                MappedMessage(
        ...
        if registerdef.get("alarmmapping") is not None:
            messages.extend(
                self.checkalarm(
                    value, registerdef.get("alarmmapping"), registertype, registerkey
                )
            )
        if registerdef.get("eventmapping") is not None:
            messages.extend(
                self.checkevent(
                    value, registerdef.get("eventmapping"), registertype, registerkey
                )
            )
